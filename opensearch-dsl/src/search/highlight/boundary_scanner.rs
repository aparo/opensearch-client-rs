...
pub enum UnifiedBoundaryScanner {
  /// Break highlighted fragments at the next sentence boundary, as determined
  /// by Java’s [BreakIterator](https://docs.oracle.com/javase/8/docs/api/java/text/BreakIterator.html).
  /// You can specify the locale to use with `boundary_scanner_locale`.
  ///
  /// > **Warning**<br/>
  /// > The `sentence` scanner splits sentences bigger than `fragment_size` at
  /// > the first word
  /// boundary next to `fragment_size`. You can set `fragment_size` to 0 to
  /// never split any sentence.
  Sentence(Option<String>),

  /// Break highlighted fragments at the next word boundary, as determined by
  /// Java’s [BreakIterator](https://docs.oracle.com/javase/8/docs/api/java/text/BreakIterator.html).
  /// You can specify the locale to use with `boundary_scanner_locale`.
  Word(Option<String>),
}
...
impl Serialize for UnifiedBoundaryScanner {
  ...
  where
    S: Serializer, {
    match self {
      Self::Sentence(locale) => {
        match locale {
          Some(locale) => {
            let mut map = serializer.serialize_struct("UnifiedBoundaryScannerSentence2", 2)?;
            map.serialize_field("boundary_scanner", "sentence")?;
            map.serialize_field("boundary_scanner_locale", locale)?;
            map.end()
          }
          None => {
            let mut map = serializer.serialize_struct("UnifiedBoundaryScannerSentence1", 1)?;
            map.serialize_field("boundary_scanner", "sentence")?;
            map.end()
          }
        }
...
mod tests {
  ...
  fn serialization() {
    ...
    assert_serialize(FvhBoundaryScanner::Chars, json!({ "boundary_scanner": "chars" }));
    ...
    assert_serialize(FvhBoundaryScanner::Word(None), json!({ "boundary_scanner": "word" }));
