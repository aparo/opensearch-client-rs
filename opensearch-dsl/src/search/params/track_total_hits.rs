...
///
/// When set to `Track` with a value `true`, the response will always track the
/// number of hits that match the query accurately.
///
/// When set to `Count` with an integer value `n`, the response accurately
/// tracks the total hit count that match the query up to `n` documents.
