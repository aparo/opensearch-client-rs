...
/// [nested](https://www.elastic.co/guide/en/opensearch/reference/current/nested.html)
/// fields.
///
...
///   Query::nested(
///     "driver.vehicle",
///     Query::term("driver.vehicle.make", "toyota"),
///   ),
/// )
/// .boost(3)
/// .name("test");
/// ```
/// <https://www.elastic.co/guide/en/opensearch/reference/current/query-dsl-nested-query.html>
...
impl Query {
  ...
  /// - `query` - Query you wish to run on nested objects in the `path`. If an
  ///   object
  /// matches the search, the `nested` query returns the root parent
  /// document.<br/> You can search nested fields using dot notation that
  /// includes the complete path, such as `obj1.name`.<br/>
...
impl NestedQuery {
  ...
  ///
  /// If `false`, OpenSearch returns an error if the `path` is an unmapped
  /// field.
  ///
  ...
  /// matches in child documents or child documents are returned based on
  /// matches in parent documents. In the nested case, documents are returned
  /// based on matches in nested inner objects.
  ///
  /// In both cases, the actual matches in the different scopes that caused a
  /// document to be returned are hidden. In many cases, it’s very useful to
  /// know which inner nested objects (in the case of nested) or
  /// children/parent documents (in the case of parent/child) caused
  /// certain information to be returned. The inner hits feature can be used for
  /// this. This feature returns per search hit in the search response
  /// additional nested hits that caused a search hit to match in a different
  /// scope.
  ///
...
deserialize_with_root!("nested": NestedQuery);
...
mod tests {
  ...
  fn serialization() {
    ...
    assert_serialize_query(
      Query::nested(
        "driver",
        Query::nested("driver.vehicles", Query::term("driver.vehicles.make.keyword", "toyota")),
      ),
      json!({
          "nested": {
              "path": "driver",
