/*
 * opensearch-client
 *
 * Rust Client for OpenSearch
 *
 * The version of the OpenAPI document: 3.1.0
 * Contact: alberto.paro@gmail.com
 * Generated by Paro OpenAPI Generator
 */

use crate::ml;
use serde::{Deserialize, Serialize};




#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct TrainPredictRequestBodyJson {  /// The input index.
    #[serde(rename = "input_index", default, skip_serializing_if = "Option::is_none")]
    pub input_index: Option<Vec<String>>,
    #[serde(rename = "parameters", default, skip_serializing_if = "Option::is_none")]
    pub parameters: Option<ml::TrainParameters>,
    #[serde(rename = "input_query", default, skip_serializing_if = "Option::is_none")]
    pub input_query: Option<ml::InputQuery>,
    #[serde(rename = "input_data", default, skip_serializing_if = "Option::is_none")]
    pub input_data: Option<ml::PredictionResult>,
}

impl TrainPredictRequestBodyJson {
    
    pub fn new() -> TrainPredictRequestBodyJson {
        TrainPredictRequestBodyJson {
            input_index: None,
            parameters: None,
            input_query: None,
            input_data: None,
        }
    }
}
