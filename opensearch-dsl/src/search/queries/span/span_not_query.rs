...
/// (controlled by the parameter `post`) another SpanQuery. The span not query
...
impl SpanNotQuery {
  /// If set the amount of tokens from within the include span can’t have
  ...
  /// If set the amount of tokens after the include span can’t have overlap with
  /// the exclude span.
  ///
  /// Defaults to 0.
  ...
  /// If set the amount of tokens before the include span can’t have overlap
...
mod tests {
  ...
  fn serialization() {
    assert_serialize_query(
      Query::span_not([Query::span_term("foo", 1234)], [Query::span_term("bar", 4321)])
        .dist(1234)
        .post(4321)
        .pre(5678),
      json!({
