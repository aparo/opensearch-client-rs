//! With scripting, you can evaluate custom expressions in OpenSearch. For
//! example, you can use a script to return a computed value as a field or
//! evaluate a custom score for a query.
//!
//! The default scripting language is [Painless](https://www.elastic.co/guide/en/opensearch/reference/current/modules-scripting-painless.html).
//! Additional `lang` plugins are available to run scripts written in other
//! languages. You can specify the language of the script anywhere that scripts
//! run.
//!
