...
/// in parent documents. In the nested case, documents are returned based on
/// matches in nested inner objects.
///
/// In both cases, the actual matches in the different scopes that caused a
/// document to be returned are hidden. In many cases, it’s very useful to know
/// which inner nested objects (in the case of nested) or children/parent
/// documents (in the case of parent/child) caused certain information to be
/// returned. The inner hits feature can be used for this. This feature returns
/// per search hit in the search response additional nested hits that caused a
