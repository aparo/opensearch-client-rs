...
/// `SpanNearQuery`. <br/> The `clauses` element is a list of one or more other
/// span type queries and the `slop` controls the maximum number of intervening
...
impl SpanNearQuery {
  ...
  /// The slop parameter allows you to specify the number of positions by which
  /// the terms in the query can be transposed to match a document. A slop
  /// value of 0 (default) means that the terms must appear in the exact order
