...
pub enum GeoBoundingBox {
    ...
    /// `bottom_right` or by `top_right` and `bottom_left` parameters. More over
    /// the names `topLeft`, `bottomRight`, `topRight` and `bottomLeft` are
    /// supported. Instead of setting the values pairwise, one can use the simple
