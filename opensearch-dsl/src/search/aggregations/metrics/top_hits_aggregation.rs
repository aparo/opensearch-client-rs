...
/// A `top_hits` metric aggregation keeps track of the most relevant document
/// being aggregated. This aggregation is intended to be used as a sub
/// aggregation, so that the top matching documents can be aggregated per
/// bucket.
///
/// > We do not recommend using `top_hits` as a top-level aggregation.
/// If you want to group search hits, use the
/// [`collapse`](https://www.elastic.co/guide/en/opensearch/reference/current/collapse-search-results.html)
/// parameter instead.
///
