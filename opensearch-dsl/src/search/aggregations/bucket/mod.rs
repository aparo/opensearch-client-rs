...
//! aggregations do, but instead, they create buckets of documents. Each bucket
//! is associated with a criterion (depending on the aggregation type) which
//! determines whether or not a document in the current context "falls" into it.
//! In other words, the buckets effectively define document sets. In addition to
//! the buckets themselves, the `bucket` aggregations also compute and return
...
pub use self::{
  date_histogram_aggregation::*, diversified_sampler_aggregation::*, filter_aggregation::*, sampler_aggregation::*,
  terms_aggregation::*,
};
