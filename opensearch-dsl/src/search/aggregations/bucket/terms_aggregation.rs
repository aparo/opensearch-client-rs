...
impl TermsAggregation {
  ...
  ///
  /// By default, the node coordinating the search process will request each
  /// shard to provide its own top `size` term buckets and once all shards
  ...
  /// term counts are slightly off and it could even be that a term that should
  /// have been in the top `size` buckets was not returned).
  pub fn size(mut self, size: u64) -> Self {
    self.terms.size = Some(size);
    self
  }

  /// Shows an error value for each term returned by the aggregation which
  /// represents the worst case error in the document count and can be useful
  /// when deciding on a value for the shard_size parameter.
  /// This is calculated by summing the document counts for the last term
  /// returned by all shards which did not return the term.
  ...
  /// > increases the
  /// [error](https://www.elastic.co/guide/en/opensearch/reference/current/search-aggregations-bucket-terms-aggregation.html#search-aggregations-bucket-terms-aggregation-approximate-counts)
  /// on document counts. It is fine when a single shard is queried, or when the
  /// field that is being aggregated was used as a routing key at index time:
  /// in these cases results will be accurate since shards have disjoint
  /// values. However otherwise, errors are unbounded. One particular case
  /// that could still be useful is sorting by [min](https://www.elastic.co/guide/en/opensearch/reference/current/search-aggregations-metrics-min-aggregation.html) or
  /// [max](https://www.elastic.co/guide/en/opensearch/reference/current/search-aggregations-metrics-max-aggregation.html)
  /// aggregation: counts will not be accurate but at least the top buckets will
  /// be correctly picked.
  ...
  /// Only returns terms that match more than a configured number of hits using
  /// the `min_doc_count`
  ///
  /// Default value is `1`
  ...
  fn serialization() {
    ...
    assert_serialize_aggregation(
      Aggregation::terms("test_field")
        ...
        .missing(123)
        .aggregate("test_sub_agg", Aggregation::terms("test_field2").size(3).missing(false)),
      json!({
          "terms": {
              "field": "test_field",
              "size": 0,
