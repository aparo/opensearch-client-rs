...
/// Performs a k-nearest neighbor (kNN) search and returns the matching
/// documents.
///
/// The kNN search API performs a k-nearest neighbor (kNN) search on a
/// `dense_vector` field. Given a query vector, it finds the _k_ closest vectors
/// and returns those documents as search hits.
///
...
/// accuracy for improved search speed. This means the results returned are
/// not always the true _k_ closest neighbors.
///
/// The kNN search API supports restricting the search using a filter. The
/// search will return the top `k` documents that also match the filter query.
///
...
impl Knn {
    ...
    ///
    /// - `field` - The name of the vector field to search against. Must be a
    ///   dense_vector field with indexing enabled.
    /// - `query_vector` - Query vector. Must have the same number of dimensions
    ...
    /// - `field` - The name of the vector field to search against. Must be a
    ///   dense_vector field with indexing enabled.
    /// - `query_vector_builder` - A configuration object indicating how to build
    ///   a query_vector before executing the request.
    ...
    /// Number of nearest neighbors to return as top hits. This value must be less
    /// than `num_candidates`.
    ///
    /// Defaults to `size`.
    ...
    /// The number of nearest neighbor candidates to consider per shard. Cannot
    /// exceed 10,000. Elasticsearch collects `num_candidates` results from each
    /// shard, then merges them to find the top results. Increasing
    /// `num_candidates` tends to improve the accuracy of the final results.
    /// Defaults to `Math.min(1.5 * size, 10_000)`.
    ...
    /// The similarity value calculated relates to the raw similarity used. Not
    /// the document score. The matched documents are then scored according
...
/// A configuration object indicating how to build a query_vector before
...
impl TextEmbedding {
    ...
    /// - `model_id` - The ID of the text embedding model to use to generate the
    ///   dense vectors from the query string. Use the same model that generated
    ///   the embeddings from the input text in the index you search against. You
    ///   can use the value of the deployment_id instead in the model_id argument.
    /// - `model_text` - The query string from which the model generates the dense
