...
/// The More Like This Query finds documents that are "like" a given set of
/// documents. In order to do so, MLT selects a set of representative terms of
/// these input documents, forms a query using these terms, executes the query
/// and returns the results. The user controls the input documents, how the
/// terms should be selected and how the query is formed.
///
/// The simplest use case consists of asking for documents that are similar to a
/// provided piece of text. Here, we are asking for all movies that have some
/// text similar to "Once upon a time" in their "title" and in their
/// "description" fields, limiting the number of selected terms to 12.
///
...
/// similar to the one used in the [Multi GET API](https://www.elastic.co/guide/en/opensearch/reference/current/docs-multi-get.html).
///
/// Finally, users can mix some texts, a chosen set of documents but also
/// provide documents not necessarily present in the index. To provide documents
/// not present in the index, the syntax is similar to [artificial documents](https://www.elastic.co/guide/en/opensearch/reference/current/docs-termvectors.html#docs-termvectors-artificial-doc).
///
/// **How it Works**
/// Suppose we wanted to find all documents similar to a given input document.
/// Obviously, the input document itself should be its best match for that type
/// of query. And the reason would be mostly, according to [Lucene scoring formula](https://lucene.apache.org/core/4_9_0/core/org/apache/lucene/search/similarities/TFIDFSimilarity.html),
...
/// document, analyzes it, usually using the same analyzer at the field,
/// then selects the top K terms with highest tf-idf to form a disjunctive query
/// of these terms.
///
...
/// ```
/// To create a `more_like_this` query with string and document id fields on
/// title and description with optional fields: ```
/// # use opensearch_dsl::queries::*;
/// # use opensearch_dsl::queries::params::*;
/// # let query =
...
///   .name("more_like_this");
/// ```
/// <https://www.elastic.co/guide/en/opensearch/reference/current/query-dsl-mlt-query.html>
...
pub enum Like {
  ...
  /// Struct to describe opensearch document which will be used in `like` field
  /// array
  Document(Document),
}
...
impl Document {
  ...
  /// The index that contains the document. Required if no index is specified in
  /// the request URI.
  ...
  /// The key for the primary shard the document resides on. Required if routing
  /// is used during indexing.
...
impl MoreLikeThisQuery {
  ...
  /// default value of *. The * value matches all fields eligible for
  /// `term-level queries`, excluding metadata fields.
  ...
  /// select terms found in a chosen set of documents. In other words, we
  /// could ask for documents like: "Apple", but unlike: "cake crumble tree".
  ...
  /// An array of stop words. Any word in this set is considered "uninteresting"
  /// and ignored. If the analyzer allows for stop words, you might want to
  /// tell MLT to explicitly ignore them, as for the purposes of document
  ...
  /// Set this to false to ignore the field and continue processing. Defaults to
  /// true.
...
mod tests {
  ...
  fn serialization() {
    ...
    assert_serialize_query(
      Query::more_like_this([Like::from(Document::new("123")), Like::from("test")]).fields(["title"]),
      json!({
          "more_like_this": {
              "fields": ["title"],
              "like": [
