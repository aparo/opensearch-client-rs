...
/// and [post_filter](https://www.elastic.co/guide/en/opensearch/reference/current/filter-search-results.html#post-filter)
...
///
/// Currently the rescore API has only one implementation: the query rescorer,
/// which uses a query to tweak the scoring. In the future, alternative
/// rescorers may be made available, for example, a pair-wise rescorer.
///
/// To create a `rescore` query with simple `term` query:
/// ```
/// # use opensearch_dsl::rescoring::*;
