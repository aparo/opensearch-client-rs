...
/// unless you have used the `stored_fields` parameter or if the `_source` field
/// is disabled.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(untagged)]
pub enum SourceFilter {
  /// Whether `_source` retrieval should be enabled (`true`) or disabled
  /// (`false`)
  Enable(bool),
  ...
  /// A collection of wildcard patterns to control what parts of `_source`
  /// should and should not be returned
  IncludesExcludes {
    /// A collection of wildcard patterns to control what parts of `_source`
    /// should be returned
    includes: Vec<String>,
