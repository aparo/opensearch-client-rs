/*
 * opensearch-client
 *
 * Rust Client for OpenSearch
 *
 * The version of the OpenAPI document: 3.1.0
 * Contact: alberto.paro@gmail.com
 * Generated by Paro OpenAPI Generator
 */

use serde::{Deserialize, Serialize};




#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct NodeBufferPool {  /// Number of buffer pools.
    #[serde(rename = "count", default, skip_serializing_if = "Option::is_none")]
    pub count: Option<u32>,
    #[serde(rename = "used", default, skip_serializing_if = "Option::is_none")]
    pub used: Option<String>,
    #[serde(rename = "used_in_bytes", default, skip_serializing_if = "Option::is_none")]
    pub used_in_bytes: Option<u32>,
    #[serde(rename = "total_capacity", default, skip_serializing_if = "Option::is_none")]
    pub total_capacity: Option<String>,
    #[serde(rename = "total_capacity_in_bytes", default, skip_serializing_if = "Option::is_none")]
    pub total_capacity_in_bytes: Option<u32>,
}

impl NodeBufferPool {
    
    pub fn new() -> NodeBufferPool {
        NodeBufferPool {
            count: None,
            used: None,
            used_in_bytes: None,
            total_capacity: None,
            total_capacity_in_bytes: None,
        }
    }
}
