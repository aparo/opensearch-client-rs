...
impl SuggestFuzziness {
  ...
  /// The fuzziness factor, defaults to [`AUTO`](Fuzziness::Auto). See
  /// [Fuzziness] for allowed settings.
  ...
  /// lengths) are measured in Unicode code points instead of in bytes. This
  /// is slightly slower than raw bytes, so it is set to `false` by default.
