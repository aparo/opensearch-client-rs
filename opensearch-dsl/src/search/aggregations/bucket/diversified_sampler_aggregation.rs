...
impl DiversifiedSamplerAggregation {
  ...
  /// The optional `execution_hint` setting can influence the management of the
  /// values used for de-duplication. Each option will hold up to `shard_size`
  /// values in memory while performing de-duplication but the type of value
  /// held can be controlled as follows:
