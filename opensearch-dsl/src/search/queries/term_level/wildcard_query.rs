...
/// For example, the `*` wildcard operator matches zero or more characters. You
/// can combine wildcard operators with other characters to create a wildcard
/// pattern.
///
...
/// ```
/// <https://www.elastic.co/guide/en/opensearch/reference/current/query-dsl-wildcard-query.html>
...
impl Query {
    ...
    /// To return a document, the wildcard must exactly match the field value,
...
impl WildcardQuery {
    ...
    /// Allows case insensitive matching of the pattern with the indexed field
    /// values when set to true. Default is false which means the case
