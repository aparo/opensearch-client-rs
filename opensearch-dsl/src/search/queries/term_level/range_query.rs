...
impl RangeQuery {
  ...
  ///
  /// By default, OpenSearch uses the
  /// [date `format`](https://www.elastic.co/guide/en/opensearch/reference/current/mapping-date-format.html)
  /// provided in the `<field>`'s mapping. This value overrides that mapping
  /// format.
  ///
  /// For valid syntax, see
  /// [`format`](https://www.elastic.co/guide/en/opensearch/reference/current/mapping-date-format.html).
  ///
  /// >If a format or date value is incomplete, the range query replaces
  /// any missing components with default values. See
  /// [Missing date components](https://www.elastic.co/guide/en/opensearch/reference/current/query-dsl-range-query.html#missing-date-components).
  ...
  /// [Time zone in `range` queries](https://www.elastic.co/guide/en/opensearch/reference/current/query-dsl-range-query.html#range-query-time-zone).
...
impl ShouldSkip for RangeQuery {
  fn should_skip(&self) -> bool {
    self.gt.should_skip() && self.gte.should_skip() && self.lt.should_skip() && self.lte.should_skip()
  }
}
  ...
  fn serialization() {
    ...
    assert_serialize_query(
      Query::range("test_date_field")
        .gt(Utc.with_ymd_and_hms(2014, 11, 28, 12, 0, 1).single().unwrap())
        .gte(Utc.with_ymd_and_hms(2014, 11, 28, 12, 0, 2).single().unwrap())
        .lt(Utc.with_ymd_and_hms(2014, 11, 28, 12, 0, 3).single().unwrap())
        .lte(Utc.with_ymd_and_hms(2014, 11, 28, 12, 0, 4).single().unwrap())
