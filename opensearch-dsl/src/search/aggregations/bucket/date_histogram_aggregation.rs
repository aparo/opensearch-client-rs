...
/// internally in OpenSearch as long values, it is possible, but not as
...
///
/// <https://www.elastic.co/guide/en/opensearch/reference/current/search-aggregations-bucket-datehistogram-aggregation.html>
...
impl DateHistogramAggregation {
  ...
  /// OpenSearch stores date-times in Coordinated Universal Time (UTC). By
  /// default, all bucketing and rounding is also done in UTC. Use the
  /// time_zone parameter to indicate that bucketing should use a different time
  /// zone.
  ...
  fn serialization() {
    ...
    assert_serialize_aggregation(
      Aggregation::date_histogram("test_field")
        ...
        .min_doc_count(2)
        .missing(Utc.with_ymd_and_hms(2014, 11, 28, 12, 0, 4).single().unwrap())
    ...
    assert_serialize_aggregation(
      Aggregation::date_histogram("test_field").aggregate("test_sub_agg", Aggregation::terms("test_field2")),
      json!({
          "date_histogram": {
              "field": "test_field",
          },
